...
namespace HogwartsMP::Core {
    ...
    std::span<const std::string_view> FastTravelLocations() {
        static const auto locations = std::to_array<std::string_view>({"Azkaban", "BothyA", "FT_AnnounceDestA", "FT_AnnounceDestB", "FT_Azkaban", "FT_BlackOffice", "FT_CentralHogsmeade", "FT_Combat_DarkArts_Entry", "FT_Combat_DarkArts_Return", "FT_DeathHallows",
            ...
            "M_FT_AVM_02_ExitTent", "M_FT_AVM_02_SkipToTent", "M_FT_BRR_01", "M_FT_BRR_02", "M_FT_BRR_03", "M_FT_EVC_Undercroft", "M_FT_EVL_Convo_01_Hamlet", "M_FT_EVL_SlytherinDungeon", "M_FT_EVZ_GreatHall", "M_FT_FGB_BlackOffice", "M_FT_FGB_HavenRoom",
    ...
    bool FastTravelTo(std::string_view name) {
        auto log = Framework::Logging::GetLogger("TeleportManager");

        // Validate against the curated list so an arbitrary script-supplied string never reaches
        // ProcessEvent and the caller gets a clear false instead of a silent in-game no-op.
        const auto locs = FastTravelLocations();
        if (std::ranges::find(locs, name) == locs.end()) {
            log->warn("FastTravelTo: unknown destination '{}'", name);
            return false;
        }

        // Cache the reflection lookups (process-stable rooted natives) — POSITIVE results only, so a
        // call made before the game has loaded these objects stays retryable instead of latching null.
        // Safe without locking: FastTravelTo runs on the game thread (see header).
        static UClass *manager        = nullptr;
        ...
        manager->ProcessEvent(getter, &instance);
        if (!instance) {
            log->warn("FastTravelManager.Get returned null");
            return false;
        }

        const auto wide = Framework::Utils::StringUtils::NormalToWide(std::string(name.data()));
        FString gname(wide.c_str());
        log->info("Teleporting to {}", name);
        instance->ProcessEvent(fastTravelTo, &gname);
        return true;
    }
} // namespace HogwartsMP::Core
